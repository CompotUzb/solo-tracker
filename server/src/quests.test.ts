...
function clock(prefix = 'id') {
  return { now: () => '2026-06-23T10:00:00.000Z', genId: () => `${prefix}-${counter++}` };
}
...
describe('quest XP reward formula', () => {
  ...
  it('exposes the canonical reward table from the parent spec', () => {
    expect(QUEST_XP_REWARDS).toEqual({ easy: 10, normal: 25, hard: 60, boss: 150, raid: 400 });
  });
...
describe('/quest add', () => {
  it('creates an active quest with the reward derived from its type', () => {
    const quest = addQuest(db, { userId: USER, title: 'Ship the engine', questType: 'hard' }, clock());
  ...
  it('requires a non-empty title and a valid type', () => {
    expect(() => addQuest(db, { userId: USER, title: '  ', questType: 'easy' })).toThrow(/title is required/);
    // @ts-expect-error invalid type on purpose
    expect(() => addQuest(db, { userId: USER, title: 'x', questType: 'mega' })).toThrow(/unknown quest type/);
  });

  it('rejects a non-positive target count', () => {
    expect(() => addQuest(db, { userId: USER, title: 'x', questType: 'easy', targetCount: 0 })).toThrow(
      /positive integer/,
    );
  });
});
...
describe('/quest complete', () => {
  it('awards XP, marks the quest done, and updates player stats', () => {
    const quest = addQuest(db, { userId: USER, title: 'Defeat the boss', questType: 'boss' }, clock());
    const result = completeQuest(db, { questId: quest.id, userId: USER }, clock());
  ...
  it('persists an auditable XP ledger row per completion', () => {
    const quest = addQuest(db, { userId: USER, title: 'Daily grind', questType: 'normal' }, clock());
    completeQuest(db, { questId: quest.id, userId: USER }, clock());
    const ledger = db
      .prepare('select user_id,source,source_id,reason,xp_delta from xp_awards where user_id=?')
      .all(USER);
    expect(ledger).toEqual([
      { user_id: USER, source: 'quest', source_id: quest.id, reason: 'quest_completed', xp_delta: 25 },
    ]);
  });

  it('is idempotent: re-completing awards no extra XP', () => {
    const quest = addQuest(db, { userId: USER, title: 'Once', questType: 'raid' }, clock());
    const first = completeQuest(db, { questId: quest.id, userId: USER }, clock());
    const second = completeQuest(db, { questId: quest.id, userId: USER }, clock());
  ...
  it('accumulates XP across quests and advances level and rank', () => {
    ...
    const boss = addQuest(db, { userId: USER, title: 'Boss', questType: 'boss' }, clock());
    const after = completeQuest(db, { questId: boss.id, userId: USER }, clock());
  ...
  it('rejects completing a missing quest or another user\'s quest', () => {
    const quest = addQuest(db, { userId: USER, title: 'Mine', questType: 'easy' }, clock());
    expect(() => completeQuest(db, { questId: 'nope', userId: USER })).toThrow(/quest not found/);
    expect(() => completeQuest(db, { questId: quest.id, userId: 'intruder' })).toThrow(/does not belong/);
...
describe('XP engine level curve', () => {
  it('maps cumulative XP to the shared level boundaries', () => {
    ...
    for (const [xp, level] of cases) {
      ...
      const r = awardXp(fresh, { userId: USER, amount: xp, reason: 'seed', source: 'test' }, clock());
  ...
  it('floors total XP at zero on a deduction and never records a level-up for it', () => {
    awardXp(db, { userId: USER, amount: 50, reason: 'grant', source: 'test' }, clock());
    const r = awardXp(db, { userId: USER, amount: -200, reason: 'penalty', source: 'test' }, clock());
  ...
  it('rejects non-integer amounts', () => {
    expect(() => awardXp(db, { userId: USER, amount: 1.5, reason: 'x', source: 'test' })).toThrow(/integer/);
  });
});

function getQuestStatus(database: Db, id: string): string {
  return (database.prepare('select status from quests where id=?').get(id) as { status: string }).status;
}
