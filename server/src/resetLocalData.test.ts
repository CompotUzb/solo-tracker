...
beforeEach(() => {
  ...
  applyMigrations(db);
  seedDatabase(db, { trackedGuildId: 'guild-1', trackedChannelIds: ['daily-channel'] }, 'test');
});
...
describe('resetLocalData', () => {
  it('clears progression data while preserving config rows and schema', () => {
    ...
    expect(db.prepare('select count(*) as n from quests').get()).toEqual({ n: 0 });
    ...
    expect(db.prepare('select count(*) as n from tracked_channels').get()).toEqual({ n: 1 });
    expect(db.prepare("select count(*) as n from sqlite_master where type='table' and name='daily_quest_days'").get()).toEqual({ n: 1 });
  });
});
