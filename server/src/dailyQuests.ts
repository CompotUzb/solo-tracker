...
import { applyStatGains, getPlayerStats, PLAYER_STAT_KEYS, type StatKey } from './stats.js';
...
const LOOT_REWARDS = {
  ...
  legendary: 'A major reward milestone (fitness apparel, books, or a tech upgrade).',
} as const;
...
export function getDailyState(db: Db, userId: string): DailyState {
  return mapState(db.prepare('select * from daily_quest_state where user_id=?').get(userId) as DailyStateRow | undefined);
}
...
function createLootBox(db: Db, userId: string, rarity: keyof typeof LOOT_REWARDS, source: string, now: string, genId: () => string) {
  db.prepare(
...
export function logDailyMetric(
  ...
  const row = db.prepare('select target,progress from daily_quest_metrics where user_id=? and local_date=? and metric_key=?').get(
    userId,
    localDate,
    metricKey,
  ) as { target: number; progress: number } | undefined;
  if (!row) throw new Error(`no daily metric ${metricKey} for ${localDate}`);

  const next = value.progress != null ? value.progress : row.progress + (value.delta ?? 0);
  const clamped = Math.max(0, Math.min(next, row.target));
  db.prepare('update daily_quest_metrics set progress=?, updated_at=? where user_id=? and local_date=? and metric_key=?').run(
    clamped,
    now,
    userId,
    localDate,
  ...
  const day = db.prepare('select status from daily_quest_days where user_id=? and local_date=?').get(userId, localDate) as
    | { status: string }
    | undefined;
...
export function runDailyEvaluation(db: Db, userId: string, todayLocalDate: string, hooks: DailyHooks = {}): EvaluationResult {
  ...
  const pastDue = db
    .prepare(`select local_date,status from daily_quest_days where user_id=? and local_date<? and evaluated=0 order by local_date asc`)
  ...
  for (const day of pastDue) {
    ...
    db.prepare(`update daily_quest_days set status='failed', evaluated=1, updated_at=? where user_id=? and local_date=?`).run(
      now,
      userId,
      day.local_date,
    );
...
export function clearDailyPenalty(db: Db, userId: string, note: string | undefined, hooks: DailyHooks = {}): DailyState {
  const now = hooks.clock?.now?.() ?? new Date().toISOString();
  ensureStateRow(db, userId, now);
  const state = getDailyState(db, userId);
  if (!state.penaltyActive) return state;
  db.prepare('update daily_quest_state set penalty_active=0, penalty_reason=null, penalty_since=null, updated_at=? where user_id=?').run(now, userId);
  hooks.notify?.({
    userId,
    type: 'system',
...
export function allocateStatPoint(db: Db, userId: string, statKey: StatKey, clock: XpClock = {}) {
  const now = clock.now?.() ?? new Date().toISOString();
  ensureStateRow(db, userId, now);
  const run = db.transaction(() => {
    const row = db.prepare('select stat_points from daily_quest_state where user_id=?').get(userId) as { stat_points: number } | undefined;
    if (!row || row.stat_points <= 0) return false;
    db.prepare('update daily_quest_state set stat_points=stat_points-1, updated_at=? where user_id=?').run(now, userId);
    applyStatGains(db, { userId, gains: [{ statKey, delta: 1 }], reason: 'stat_point_allocation', source: 'allocation' }, clock);
    return true;
  });
  const ok = run();
  return { ok, state: getDailyState(db, userId), stats: getPlayerStats(db, userId).stats };
}
...
export function listLootBoxes(db: Db, userId: string, limit = 20): LootBox[] {
  const rows = db
    .prepare('select * from loot_boxes where user_id=? order by (status=\'unopened\') desc, created_at desc limit ?')
    .all(userId, Math.min(Math.max(limit, 1), 100)) as LootBoxRow[];
  return rows.map(mapLoot);
}

export function claimLootBox(db: Db, userId: string, id: string, clock: XpClock = {}): LootBox | null {
  const now = clock.now?.() ?? new Date().toISOString();
  db.prepare(`update loot_boxes set status='claimed', claimed_at=? where id=? and user_id=? and status='unopened'`).run(now, id, userId);
  const row = db.prepare('select * from loot_boxes where id=? and user_id=?').get(id, userId) as LootBoxRow | undefined;
