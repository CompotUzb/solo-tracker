...
import { applyMigrations, openDatabase, seedDatabase, SEED_USER_ID, type Db } from './db.js';
...
describe('schema', () => {
  it('creates every required table', () => {
    ...
    try {
      ...
      for (const t of REQUIRED_TABLES) expect(tables.has(t), `missing table: ${t}`).toBe(true);
    } finally {
      db.close();
    }
  });
  ...
  it('seeds the player/user row and the configured channel whitelist', () => {
    const db = freshDb();
    try {
      const config = { trackedGuildId: 'guild-1', trackedChannelIds: ['chan-1', 'chan-2'], timezone: 'Asia/Tashkent' };
      seedDatabase(db, config, '002_rpg_schema.sql');

      const user = db.prepare('select user_id,is_player from users where user_id=?').get(SEED_USER_ID) as
        | { user_id: string; is_player: number }
        | undefined;
      expect(user).toBeDefined();
      expect(user?.is_player).toBe(1);

      const channels = db.prepare('select channel_id from tracked_channels order by channel_id').all() as {
        channel_id: string;
      }[];
  ...
  it('seeding is idempotent', () => {
    const db = freshDb();
    try {
      const config = { trackedGuildId: 'guild-1', trackedChannelIds: ['chan-1'], timezone: 'Asia/Tashkent' };
      seedDatabase(db, config, '002_rpg_schema.sql');
      seedDatabase(db, config, '002_rpg_schema.sql');

      const userCount = db.prepare('select count(*) as n from users').get() as { n: number };
      expect(userCount.n).toBe(1);
      const channelCount = db.prepare('select count(*) as n from tracked_channels').get() as { n: number };
