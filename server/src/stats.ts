...
const SECONDARY_KEYWORDS: Record<ChannelCategory, Partial<Record<StatKey, string[]>>> = {
  'daily-quests': {
    health: ['sleep', 'food', 'eat', 'meal', 'exercise', 'workout', 'recovery', 'rest', 'hydrate', 'water'],
    intelligence: ['study', 'read', 'learn', 'reason', 'analy', 'research'],
    technical: ['code', 'coding', 'program', 'bug', 'deploy', 'build'],
  },
  'mind-training': {
    communication: ['writ', 'speak', 'language', 'english', 'vocab', 'essay', 'journal', 'blog'],
    survival: ['decision', 'risk', 'surviv', 'practical', 'real-world', 'prepare', 'plan'],
    discipline: ['focus', 'consistent', 'habit', 'routine', 'discipline', 'streak'],
  },
  'body-training': {
    health: ['health', 'recovery', 'rest', 'sleep', 'nutrition', 'diet', 'stretch'],
    discipline: ['consistent', 'habit', 'routine', 'discipline', 'streak', 'daily'],
    survival: ['surviv', 'practical', 'readiness', 'endurance', 'conditioning'],
  },
  'work-skill': {
    wealth: ['wealth', 'career', 'money', 'client', 'invoice', 'salary', 'business', 'revenue', 'ship', 'launch'],
    communication: ['report', 'meeting', 'message', 'email', 'present', 'standup', 'doc'],
 ...
 */
export function messageStatGains(category: ChannelCategory, contentLength: number, content = ''): StatGain[] {
  ...
  if (text) {
    for (const [statKey, keywords] of Object.entries(SECONDARY_KEYWORDS[category]) as [StatKey, string[]][]) {
      if (statKey === primary) continue;
      if (keywords.some((kw) => text.includes(kw))) gains.push({ statKey, delta: 1 });
    }
  }
  return gains;
}
...
export function questStatGains(questType: string): StatGain[] {
  const amount: Record<string, number> = { easy: 2, normal: 2, hard: 3, boss: 4, raid: 5 };
 ...
 */
export function applyStatGains(db: Db, input: ApplyStatGainsInput, clock: XpClock = {}): ApplyStatGainsResult {
  ...
  const run = db.transaction((): ApplyStatGainsResult => {
    ...
    for (const gain of input.gains) {
      ...
      const row = db.prepare('select value from player_stats where user_id=? and stat_key=?').get(input.userId, gain.statKey) as
        | { value: number }
        | undefined;
      ...
      db.prepare(
        `insert into stat_awards (id,user_id,stat_key,delta,reason,source,source_id,occurred_at,created_at)
         values (?,?,?,?,?,?,?,?,?)`,
      ).run(genId(), input.userId, gain.statKey, applied, input.reason, input.source, input.sourceId ?? null, now, now);
...
export function awardMessageStats(
  db: Db,
  params: { userId: string; category: ChannelCategory; contentLength: number; content?: string; localDate: string; sourceId?: string | null },
  clock: XpClock = {},
): ApplyStatGainsResult | null {
  const gains = messageStatGains(params.category, params.contentLength, params.content);
  if (!gains.length) return null;

  const dayStart = `${params.localDate}T00:00:00.000Z`;
  const used = db
    .prepare(`select count(*) as n from stat_awards where user_id=? and source=? and reason=? and occurred_at>=?`)
    .get(params.userId, 'discord', `message:${params.category}`, dayStart) as { n: number };
  if (used.n >= MAX_MESSAGE_STAT_GAINS_PER_CATEGORY_PER_DAY) return null;

  return applyStatGains(
    db,
    { userId: params.userId, gains, reason: `message:${params.category}`, source: 'discord', sourceId: params.sourceId ?? null },
    clock,
  );
}
