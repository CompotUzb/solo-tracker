...
describe('message stat gains', () => {
  it('grants the channel primary stat for a meaningful log', () => {
    expect(messageStatGains('body-training', 20)).toEqual([{ statKey: 'strength', delta: 1 }]);
    expect(messageStatGains('work-skill', 20)).toEqual([{ statKey: 'technical', delta: 1 }]);
  });
  ...
  it('adds a secondary stat only when content keywords match', () => {
    const withContent = messageStatGains('work-skill', 40, 'shipped client invoice and wrote the report');
    ...
    expect(messageStatGains('work-skill', 40)).toEqual([{ statKey: 'technical', delta: 1 }]);
  });
});

describe('quest stat gains', () => {
  it('scales discipline by difficulty', () => {
    expect(questStatGains('normal')).toEqual([{ statKey: 'discipline', delta: 2 }]);
    expect(questStatGains('hard')).toEqual([{ statKey: 'discipline', delta: 3 }]);
    expect(questStatGains('raid')).toEqual([{ statKey: 'discipline', delta: 5 }]);
  });
});
...
describe('applyStatGains', () => {
  it('materializes values and writes an audit ledger row', () => {
    ...
    try {
      ...
      const result = applyStatGains(
        db,
        { userId: USER, gains: [{ statKey: 'strength', delta: 1 }], reason: 'message:body-training', source: 'discord' },
        clock,
      );
      ...
      const ledger = db.prepare('select count(*) as n from stat_awards where user_id=?').get(USER) as { n: number };
  ...
  it('floors stat values at zero', () => {
    const db = freshDb();
    try {
      applyStatGains(db, { userId: USER, gains: [{ statKey: 'health', delta: -5 }], reason: 'penalty', source: 'manual' });
      expect(getPlayerStats(db, USER).stats.find((s) => s.key === 'health')?.value).toBe(0);
    } finally {
      db.close();
    }
  });
...
describe('awardMessageStats daily cap', () => {
  it('stops awarding once the per-category daily cap is reached', () => {
    ...
    try {
      const clock = { now: () => '2026-06-23T10:00:00.000Z', genId: counter() };
      let awarded = 0;
      for (let i = 0; i < MAX_MESSAGE_STAT_GAINS_PER_CATEGORY_PER_DAY + 3; i++) {
        const result = awardMessageStats(
          db,
          { userId: USER, category: 'mind-training', contentLength: 30, localDate: '2026-06-23', sourceId: `m-${i}` },
          clock,
        );
        if (result) awarded++;
      }
      expect(awarded).toBe(MAX_MESSAGE_STAT_GAINS_PER_CATEGORY_PER_DAY);
      expect(getPlayerStats(db, USER).stats.find((s) => s.key === 'intelligence')?.value).toBe(
        MAX_MESSAGE_STAT_GAINS_PER_CATEGORY_PER_DAY,
      );
    } finally {
      db.close();
