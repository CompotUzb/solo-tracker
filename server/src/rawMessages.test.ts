...
describe('raw Discord message persistence', () => {
  it('stores message id, channel id, author id, content, and timestamp', () => {
    ...
    try {
      ...
      const row = db.prepare('select * from raw_messages where message_id=?').get('msg-1') as Record<string, unknown>;
      ...
      expect(JSON.parse(row.metadata_json as string)).toEqual({ attachmentCount: 0 });
    } finally {
      db.close();
    }
  });
  ...
  it('does not duplicate rows for duplicate message ids', () => {
    ...
    try {
      ...
      expect(storeRawMessage(db, { ...message, content: 'duplicate version' })).toBe(false);

      const rows = db.prepare('select content from raw_messages where message_id=?').all('msg-dup');
