...
import { createNotifier, listNotifications, recordNotification } from './notifications.js';
...
const base = { userId: 'local-user', type: 'level_up' as const, title: 'Level 2 reached' };
...
describe('notifications storage', () => {
  it('stores a notification and lists it back', () => {
    ...
    try {
      ...
      expect(list[0]).toMatchObject({ type: 'level_up', title: 'Level 2 reached', body: 'Nice work', discordStatus: 'skipped' });
    } finally {
      db.close();
    }
  });
...
describe('notifier', () => {
  ...
  it('marks the row failed when delivery throws', async () => {
    ...
    try {
      ...
      await vi.waitFor(() => {
        expect(listNotifications(db, 'local-user')[0].discordStatus).toBe('failed');
      });
