...
const boundary: BoundaryConfig = { trackedGuildId: 'guild-1', trackedChannelIds: ['channel-1'] };
...
describe('raw Discord message ingestion', () => {
  it('maps a Discord message into a raw-message input with metadata', () => {
    const raw = toRawMessageInput(fakeMessage({ id: 'msg-9', guildId: 'guild-1', channelId: 'channel-1', content: 'studied 1h' }), {
      storeMessageContent: true,
      contentMaxChars: 7,
    });
  ...
  it('omits content by default and stores only content length metadata', () => {
    const raw = toRawMessageInput(fakeMessage({ id: 'msg-private', guildId: 'guild-1', channelId: 'channel-1', content: 'private details' }));

    expect(raw.content).toBe('');
    expect(raw.metadata).toMatchObject({ attachmentCount: 0, contentLength: 15 });
  });
  ...
  it('persists tracked human guild messages only and emits a live hook for stored messages', () => {
    ...
    const emitMessageCreate = (message: Message) => (client as { emit(event: string, ...args: unknown[]): boolean }).emit('messageCreate', message);

    emitMessageCreate(fakeMessage({ id: 'tracked', guildId: 'guild-1', channelId: 'channel-1' }));
    emitMessageCreate(fakeMessage({ id: 'bot', guildId: 'guild-1', channelId: 'channel-1', author: { id: 'bot-1', bot: true } }));
    emitMessageCreate(fakeMessage({ id: 'dm', guildId: null, channelId: 'dm-channel' }));
    emitMessageCreate(fakeMessage({ id: 'other-channel', guildId: 'guild-1', channelId: 'other' }));

    expect(stored).toHaveLength(1);
    expect(stored[0]).toMatchObject({ messageId: 'tracked', channelId: 'channel-1', authorId: 'user-1', content: 'progress update' });
    expect(liveEvents).toHaveLength(1);
    expect(liveEvents[0]).toMatchObject({ input: { messageId: 'tracked', channelId: 'channel-1' }, result: true });
    client.destroy();
  });
});
