...
function add(matches: ParsedDailyMetric[], metricKey: DailyMetricKey, amount: number, rawMatch: string): void {
  if (Number.isFinite(amount) && amount > 0) matches.push({ metricKey, amount, rawMatch });
}
...
export function parseDailyProgress(content: string): ParsedDailyMetric[] {
  ...
  for (const match of content.matchAll(/\b(?:studied|study|focused|focus|meditated|meditate)\s+(?:for\s+)?(\d+(?:\.\d+)?)\s*(?:m|min|mins|minutes?)\b/gi)) {
    add(matches, 'mental_minutes', Number(match[1]), match[0]);
  }
  for (const match of content.matchAll(/\bread\s+(\d+(?:\.\d+)?)\s*pages?\b/gi)) {
  ...
  for (const match of matches) {
    ...
    totals.set(match.metricKey, {
      ...
      rawMatch: current ? `${current.rawMatch}, ${match.rawMatch}` : match.rawMatch,
    });
  }
...
export function getDailyQuestTierForRank(rank: string): DailyTier {
  const normalized = rank.trim().toLowerCase().replace(/[\s_-]+/g, '');
  if (['s', 'srank', 'national', 'nationallevel', 'nationallevelhunter', 'monarch'].includes(normalized)) return 's';
  if (['c', 'crank', 'b', 'brank', 'a', 'arank'].includes(normalized)) return 'c';
  return 'e';
}

export function resolveDailyQuestTier(rank: string, override: number | null = null): DailyTier {
...
export function formatDailyQuestMessage(streakDayNumber: number, hunterRank: string, tier: DailyTier): string {
...
export async function createDailyQuestForDate(input: {
  ...
  const quest = ensureWorkflowDay(input.db, input.userId, input.localDate, input.hunterRank, tier, now);
  if (quest.discordParentMessageId && quest.discordThreadId) return { created: false, quest };
  ...
  input.db.prepare(
    `update daily_quest_days
      ...
      where user_id=? and local_date=?`,
  ).run(published.parentMessageId, published.threadId, published.threadName, now, input.userId, input.localDate);
  return { created: true, quest: getDailyQuest(input.db, input.userId, input.localDate)! };
}

export function getActiveDailyQuestByThread(db: Db, threadId: string): DailyQuestView | null {
  const row = db
    .prepare(`select user_id,local_date from daily_quest_days where discord_thread_id=? and status='active' order by local_date desc limit 1`)
...
export function recordDailyThreadMessage(input: {
  ...
  if (!quest || quest.id == null || quest.date == null) return { accepted: false, parsed: [], quest: null, completion: null };
  const parsed = parseDailyProgress(input.content);
  if (parsed.length === 0) return { accepted: true, parsed, quest, completion: null };
  ...
  for (const metric of parsed) {
    const inserted = input.db.prepare(
      `insert or ignore into daily_quest_metric_events
       ...
       values (?,?,?,?,?,?,?)`,
    ).run(randomUUID(), quest.id, input.messageId, metric.metricKey, metric.amount, input.storeRawMatch ? metric.rawMatch : null, now);
    if (inserted.changes === 0) continue;
    const result = logDailyMetric(input.db, input.userId, quest.date, metric.metricKey, { delta: metric.amount }, input.hooks);
    completion ??= result.completion;
  }
  return { accepted: true, parsed, quest: getDailyQuest(input.db, input.userId, quest.date), completion };
}
...
export function millisecondsUntilLocalTime(now: Date, timezone: string, localTime: string): number {
  ...
  const currentSeconds = (part('hour') % 24) * 3600 + part('minute') * 60 + part('second');
  const [hour, minute] = localTime.split(':').map(Number);
  const targetSeconds = hour * 3600 + minute * 60;
  const deltaSeconds = targetSeconds > currentSeconds ? targetSeconds - currentSeconds : 86400 - currentSeconds + targetSeconds;
  return deltaSeconds * 1000 + 1000;
}

export function hasReachedLocalTime(now: Date, timezone: string, localTime: string): boolean {
  const parts = new Intl.DateTimeFormat('en-US', {
    timeZone: timezone,
    hour12: false,
    hour: '2-digit',
