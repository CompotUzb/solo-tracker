...
describe('quest API routes', () => {
  it('adds a quest, completes it, and reflects awarded XP in stats', async () => {
    ...
    expect(quest).toMatchObject({ questType: 'boss', xpReward: 150, status: 'active' });
    ...
    const list = await api.app.inject({ method: 'GET', url: '/api/quests?status=completed' });
  ...
  it('returns 404 for an unknown quest completion', async () => {
    const api = makeApi();
    const res = await api.app.inject({ method: 'POST', url: '/api/quests/missing/complete', payload: {} });
