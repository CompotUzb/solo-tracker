...
describe('weeklyReport', () => {
  it('summarizes seven days of stats and buckets completed quests by local date', () => {
    ...
    insertStats.run(USER, today, 5, 1, 40, 1, now.toISOString(), now.toISOString());
    insertStats.run(USER, earlier, 2, 1, 15, 1, now.toISOString(), now.toISOString());
    ...
    ).run(randomUUID(), USER, 'Daily grind', 'normal', 25, `${today}T09:00:00.000Z`, now.toISOString(), now.toISOString());
    ...
    expect(todayEntry).toEqual({ date: today, messages: 5, xp: 40, questsCompleted: 1 });
  });

  it('returns all-zero days for a user with no data', () => {
    db = freshDb();
    const report = weeklyReport(db, USER, TZ, new Date('2026-06-23T12:00:00.000Z'));
    expect(report.days).toHaveLength(7);
    expect(report.totals).toEqual({ messages: 0, xp: 0, questsCompleted: 0, activeDays: 0 });
  });
});
...
describe('listAchievements', () => {
  it('orders unlocked achievements before in-progress ones', () => {
    ...
    insert.run(randomUUID(), USER, 'first_blood', 'First Blood', 'Post your first message', 'bronze', 1, 1, now, now, now);
    insert.run(randomUUID(), USER, 'marathon', 'Marathon', 'Stay active 30 days', 'gold', 4, 30, null, now, now);

    const achievements = listAchievements(db, USER);
    expect(achievements).toHaveLength(2);
    expect(achievements[0]).toMatchObject({ code: 'first_blood', unlocked: true });
    expect(achievements[1]).toMatchObject({ code: 'marathon', unlocked: false, progress: 4, target: 30 });
  });
});
...
describe('read-only report routes', () => {
  it('serves achievements and the weekly report', async () => {
    ...
    expect(achievements.json()).toMatchObject({ userId: 'local-user', achievements: [] });

    const weekly = await api.app.inject({ method: 'GET', url: '/api/reports/weekly' });
