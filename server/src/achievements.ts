...
function unlockAchievement(
  ...
  const result = db
    ...
    .run(id, userId, definition.code, definition.name, definition.description, definition.tier, definition.target, now, now, now);
  if (result.changes !== 1) return null;

  const row = db.prepare('select id,user_id,code,name,description,tier,unlocked_at from achievements where id=?').get(id) as AchievementRow;
  return {
    id: row.id,
    userId: row.user_id,
    code: row.code,
...
export function checkAchievementsAfterQuestCompleted(db: Db, quest: Quest, xpAwarded: number): AchievementUnlock[] {
  const completedCount = db
    .prepare(`select count(*) as n from quests where user_id=? and status='completed'`)
...
export function checkAchievementsAfterLevelUp(db: Db, award: AwardXpResult): AchievementUnlock[] {
  if (!award.leveledUp) return [];
  const unlocked = unlockAchievement(db, award.userId, DEFINITIONS.firstLevelUp, {
...
export function achievementNotification(unlock: AchievementUnlock): NotificationInput {
  if (unlock.code === DEFINITIONS.firstQuest.code) {
    return {
      ...
      metadata: { achievementCode: unlock.code, achievementId: unlock.id, ...unlock.context },
    };
  }
